...
function Payment(){
  const dispatch=useDispatch();
    ...
    const {workshop}=useSelector(state=>state.workshopState);
...
const formattedDateTime = `${year}-${month}-${day} ${hours}:${minutes}:${seconds}`;
  ...
  const successEndpoint = `https://gyanmitra24.onrender.com/api/v1/payment/success?userId=${user.user_id}&userName=${encodeURIComponent(user.name)}&userEmail=${encodeURIComponent(user.email)}&workshopId=${encodeURIComponent(workshop.workshopid)}&workshopName=${encodeURIComponent(workshop.name)}&status=${encodeURIComponent("Success")}&timestamp=${encodeURIComponent(formattedDateTime)}&hash=${hash}&transactionId=${transactionID}`;
const failureEndpoint = `https://gyanmitra24.onrender.com/api/v1/payment/failure?userId=${user.user_id}&userName=${encodeURIComponent(user.name)}&userEmail=${encodeURIComponent(user.email)}&workshopId=${encodeURIComponent(workshop.workshopid)}&workshopName=${encodeURIComponent(workshop.name)}&status=${encodeURIComponent("Failure")}&timestamp=${encodeURIComponent(formattedDateTime)}&hash=${hash}&transactionId=${transactionID}`;
...
<label >Email:</label>
        ...
        <input
          type="text"
          id="user_amount_pay"
          value={workshop.registration_fee}
          disabled
        />
